#!/usr/bin/env bash
# Preflight for data_acquisition — validates the environment the NEXT run will
# actually use. Fleet paradigm (docs/migration_CLAUDE.md); adapted from the
# mmb-rpp reference. A clean run reports ZERO warnings: treat a persistent
...
# Read KEY= from .env, stripping dotenv-style inline comments and trailing
# whitespace (a trailing "  # note" must never become part of a password).
env_val() {
...
if [ -d "$LOG_DIR" ] && [ -w "$LOG_DIR" ]; then
    ...
    # entrypoint.sh repairs a MISSING dev dir on first containerized run, so
    # missing-in-dev is only a warning; a missing release dir is a real error.
    case "$LOG_DIR" in
        /opt/run-logs/*) error "LOG_DIR $LOG_DIR does not exist — create it svc:docker 2775 before the first release run" ;;
        *) warn "LOG_DIR $LOG_DIR does not exist yet (entrypoint.sh creates it on first docker run)" ;;
    esac
fi
...
SSH_KEY="$(env_val SSH_KEY)"
if [ -z "$SSH_KEY" ]; then
    error "SSH_KEY empty in .env — rsync/mmb jobs cannot authenticate"
elif [ -e "/opt/resources/ssh/$SSH_KEY" ]; then
    ok "SSH key present: /opt/resources/ssh/$SSH_KEY"
else
    error "SSH key not found at /opt/resources/ssh/$SSH_KEY"
fi
...
if [ ! -f .env ]; then
    ...
    REQUIRED="APP_NAME USER_ID LOGGER_MODE LOG_DIR PGHOST PGPORT PGUSER PGPASSWORD PGDATABASE PG_SSLMODE PG_SSL_PATH REDIS_HOST REDIS_PORT REDIS_PW APP_SECRET SSH_KEY DATA_STORE_DEV"
    ...
    # VNS3 is only needed by the ip_reset/tunnel jobs — presence-only check.
    for key in VNS3_IP VNS3_PW; do
        [ -n "$(env_val "$key")" ] || warn ".env: $key empty — ip_reset/tunnel jobs will fail (ignore if not scheduled here)"
    done
    for retired in IMAGE_TAG RUN_ENV RUN_LOGS_DIR NODE_MOD_CACHE_DEV LOGGER RUN_USER; do
...
for f in index.js package.json logger.js docker/Dockerfile docker/entrypoint.sh docker-compose.yaml build.sh build-release.sh; do
    if [ -f "$f" ]; then ok "$f present"; else error "$f missing"; fi
done
for d in utils/db utils/logger utils/sh; do
