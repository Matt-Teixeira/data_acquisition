...
| — | Independent doc-2.1 audit (A21) | ✅ Findings addressed | 2026-08-18 | Second unbiased Claude pass (`AUDIT_DOC21_2026-08-18.md`): 15 findings, all verified true except A21-02 (auditor couldn't see the source/prod server — seed script exists there) and A21-04 (owner created pg_manage_v2 DEV branch same day). Fixed: new STEP 1.4 (GitHub SSH + infra clones + git/psql install), edu-schema correction, acceptance/baseline scoping, monitoring out-of-scope + conditional uid-999 checks, odd-jobs go-live step, loopback-trust note, schedules.md verbatim refresh + svc section, identity placeholders in 11 repos' env examples (incl. monday's REL-02-recreating `PGDATABASE=dev`), RUN_ENV set in acumatica/psp, stale redis-admin .env password keys deleted. Privileged-verification list (§3 of the audit) remains for Matt |
