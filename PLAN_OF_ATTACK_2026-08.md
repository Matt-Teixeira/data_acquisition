...
| 2 | Password rotation + scrub | ✅ Done | 2026-08-17 | Rotated via \password + 9 env keys; pre-rotation sessions killed (incl. idle superuser conn from Jun 25); all 8 apps smoke-tested OK; 5 repos scrubbed + dead script deleted (commits = Matt). Open: SFTP vendor rotation, D3 history cleanup, secret scanner (Phase 2) |
...
| 12 | Doc 2.1 written | ◐ Drafted | 2026-08-18 | Full draft at `docs/docker_server_full_setup_2.1.md`, every fact live-verified same day (branches/upstreams, build matrix, redis auth, pg compose, schedules). New sections: outcome contract, partition maintenance (odd-jobs), security baseline, B0a, expanded acceptance checklist. REL-07 placeholder fix applied+committed in all 8 repos to make the doc true. **Awaiting Matt's review before commit** |
